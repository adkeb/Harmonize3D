...
class AutoSceneTest(unittest.TestCase):
    ...
    def test_final_position_retry_plan_uses_white_model_channels_only(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            roles = {entry["role"] for entry in item["input_images"]}
            self.assertIn("white_model_rgb_position_lock", roles)
            self.assertIn("position_contract_measurement_reference", roles)
            self.assertNotIn("appearance_style_reference_only", roles)
            measurement = next(entry for entry in item["input_images"] if entry["role"] == "position_contract_measurement_reference")
            self.assertTrue(Path(measurement["path"]).exists())
            self.assertEqual(item["measurement_reference_role"], "position_contract_measurement_reference")
            self.assertIn("bbox and empty-margin measurement guide", item["prompt"])
            ...
            self.assertIn("position_contract_measurement_reference", handoff_text)
